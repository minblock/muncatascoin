...
const char * const BITCOIN_CONF_FILENAME = "muncatascoin.conf";
...
#else
    const char* pszModule = "muncatascoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Muncatascoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Muncatascoin";
#else
    // Unix
    return pathRet / ".muncatascoin";
#endif
#endif
}
